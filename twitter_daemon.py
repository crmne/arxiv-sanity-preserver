...
while True:
    ...
    for r in results:
        ...
        arxiv_pids = [p for p in arxiv_pids
                      if p in db]  # filter to those that are in our paper db
        ...
        tweet['created_at_time'] = (
            d - epochd).total_seconds()  # seconds since epoch
        tweet['lang'] = r.lang
        tweet['text'] = r.text
    ...
    for days in [1, 7, 30]:
        ...
        for tweet in relevant:
            ...
            for pid in tweet['pids']:
                ...
                records_dict[pid]['tweets'].append({
                    ...
                    'image_url':
                    tweet['user_image_url'],
                    'text':
                    tweet['text'],
                    'weight':
                    float_vote + tweet_sort_bonus,
                    'ok':
                    isok2,
                    'id':
                    str(tweet['id'])
        ...
        for pid in votes:
            records_dict[pid]['vote'] = votes[
                pid]  # record the total amount of vote across relevant tweets
        ...
        for pid, d in records_dict.items():
            ...
            if len(d['tweets']) > max_tweet_records:
                d['tweets'] = d['tweets'][:max_tweet_records]

        # some debugging information
        votes = [(v, k) for k, v in votes.items()]
        votes.sort(reverse=True,
                   key=lambda x: x[0])  # sort descending by votes
        ...
        if records_dict:
            ...
            )  # insert all precomputed records (minimal tweets) with their votes
