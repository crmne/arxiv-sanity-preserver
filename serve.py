...
def get_username(user_id):
    ...
    rv = query_db('select username from user where user_id = ?', [user_id],
                  one=True)
    return rv[0] if rv else None
...
def discuss():
    ...
    # fetch the comments
    comms_cursor = comments.find({
        'pid': pid
    }).sort([('time_posted', pymongo.DESCENDING)])
...
def discussions():
    ...
    comms_cursor = comments.find().sort([('time_posted', pymongo.DESCENDING)
                                         ]).limit(100)
...
def recommend():
    ...
    ctx = default_context(
        ...
        if g.user else
        'You must be logged in and have some papers saved in your library.')
    return render_template('main.html', **ctx)
...
def top():
    ...
    papers = [
        p for p in top_sorted_papers
        if curtime - p['time_published'] < tt * 24 * 60 * 60
    ]
    papers = papers_filter_version(papers, vstr)
    ctx = default_context(papers,
                          render_format='top',
...
def toptwtr():
    ...
    ctx = default_context(papers,
                          ...
                          tweets=tweets,
                          msg='Top papers mentioned on Twitter over last ' +
                          ttstr + ':')
    return render_template('main.html', **ctx)
...
def library():
    ...
    ret = encode_json(
        papers, 500)  # cap at 500 papers in someone's library. that's a lot!
    if g.user:
...
def friends():
    ...
    if g.user:
        ...
        keys.sort(key=lambda k: len(counts[k]),
                  reverse=True)  # descending by count
        papers = [db[x] for x in keys]
        # finally filter by date
        curtime = int(time.time())  # in seconds
        papers = [
            x for x in papers
            if curtime - x['time_published'] < tt * 24 * 60 * 60
        ]
        # trim at like 100
        if len(papers) > 100: papers = papers[:100]
        # trim counts as well correspondingly
        pid_to_users = {
            p['_rawid']: counts.get(p['_rawid'], [])
            for p in papers
        }
    ...
    ctx = default_context(papers,
                          render_format='friends',
                          pid_to_users=pid_to_users,
                          msg=msg)
    return render_template('main.html', **ctx)
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(Config.database_path):
        print(
            'did not find as.db, trying to create an empty database from schema.sql...'
        )
    ...
    TAGS = [
        'insightful!', 'thank you', 'agree', 'disagree', 'not constructive',
        'troll', 'spam'
    ]
